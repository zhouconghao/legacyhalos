...
def _start(galaxy, log=None, seed=None):
    ...
    print(
        "Started working on galaxy {} at {}".format(galaxy, time.asctime()),
        flush=True,
        file=log,
    )
...
def _done(galaxy, galaxydir, err, t0, stage, filesuffix=None, log=None):
    ...
    print(
        "Finished galaxy {} in {:.3f} minutes.".format(galaxy, (time.time() - t0) / 60),
        flush=True,
        file=log,
    )
    return
...
def call_sersic(onegal, galaxy, galaxydir, seed, verbose, debug, logfile):
    ...
    if debug:
        ...
        err = legacyhalos.sersic.legacyhalos_sersic(
            onegal,
            galaxy=galaxy,
            galaxydir=galaxydir,
            debug=debug,
...
def call_sky(
    ...
    if debug:
        _start(galaxy, seed=seed)
        err = legacyhalos.sky.legacyhalos_sky(
            onegal,
            survey=survey,
            galaxy=galaxy,
            galaxydir=galaxydir,
...
def call_custom_coadds(
    ...
    #     print(f"{galaxy} failed in custom_coadds", flush=True)
    #     return
